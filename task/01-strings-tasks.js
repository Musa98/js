...
function getRectangleString(width, height) {
    var main = '┌' + '─'.repeat(width - 2) + '┐\n';
	
	for (var i=1;i <= height - 2; i++){
		main += '│' + ' '.repeat(width - 2) + '│\n'; 
	}
	
	return main += '└' + '─'.repeat(width - 2) + '┘\n';
}
...
function encodeToRot13(str) {
    var symbols1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
	var symbols2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
	var resultstr = '';
	
	for (var i = 0; i < str.length; i++){
		var value = symbols1.indexOf(str[i]);
		
		if (value != -1){
			resultstr += symbols2[value];
		}
...
function getCardId(value) {
	...
	var picture = ['♣','♦','♥','♠'];
	return card.indexOf(value.slice(0,value.length - 1)) + 13 * picture.indexOf(value.slice(value.length-1))
}
